...
public class ServerSetting {
	...
	/** 根目录 */
	private static File root = new File("./fs/");
	/** Filter映射表 */
	private static Map<String, Filter> filterMap = new ConcurrentHashMap<>();
	/** Action映射表 */
	private final static Map<String, Class<? extends Action>> getActionMap = new ConcurrentHashMap<>();
    private final static Map<String, Class<? extends Action>> postActionMap = new ConcurrentHashMap<>();
    private final static Map<String, Class<? extends Action>> putActionMap = new ConcurrentHashMap<>();
    private final static Map<String, Class<? extends Action>> deleteActionMap = new ConcurrentHashMap<>();
    private final static Map<String, Class<? extends Action>> errorActionMap = new ConcurrentHashMap<>();
    ...
    public static Action getErrorAction(String path) {
        Class cls = errorActionMap.get(path);
        if(cls != null) {
            try {
	...
	public static Action getAction(String path, String method){
		...
		Class cls = getActionMap(method).get(path.trim());
        if(cls != null) {
            try {
                return (Action) cls.newInstance();
            } catch (InstantiationException e) {
	...
	public static void setAction(String path, Action action) {
		...
		ServerSetting.getActionMap(method).put(path, action.getClass());
	}
